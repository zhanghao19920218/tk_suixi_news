             ...
             failure:(void (^)(NSError * _Nonnull))failureHander
{
    ...
    switch (params.method) {
        case TGHttpMethodGet:
        {
            ...
            NSString *token = [tokenid objectForKey:K_JT_token];
            [manager.requestSerializer setValue:token forHTTPHeaderField:@"Authorization"];
            [manager.requestSerializer willChangeValueForKey:@"timeoutInterval"];
            manager.requestSerializer.timeoutInterval = 15.0f;
            [manager.requestSerializer didChangeValueForKey:@"timeoutInterval"];
            [manager.requestSerializer setValue:[[NSUserDefaults standardUserDefaults] objectForKey:@"SESSION_ID"] forHTTPHeaderField:@"X-SESSIONID"];
            NSURLSessionTask *task = [manager GET:formattedUrl parameters:paramsDic progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
                if (!responseObject)
                {
                ...
                } else
                {
                    if (successHandler) {
                        ...
                        if ([responseObject isKindOfClass:[NSDictionary class]]){
                            ...
                            //判断status == 2的内容remove Token
                            NSString *status = [responseObject objectForKey:K_JT_code];
                            if (status.integerValue == 2)
                            {
                                [TGApplicationPrefix userSignOutActionFromToken];
                            }
                        }
        ...
        case TGHttpMethodPost:
        {
            ...
            //将token封装入请求头
            manager.requestSerializer = [AFJSONRequestSerializer serializer];
            [manager.requestSerializer setValue:[[NSUserDefaults standardUserDefaults] objectForKey:K_JT_token] forHTTPHeaderField:@"Authorization"];
            manager.responseSerializer = [AFJSONResponseSerializer serializerWithReadingOptions:NSJSONReadingMutableContainers];
            manager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"application/json",@"text/html", @"text/json", @"text/javascript",@"image/jpeg",@"text/plain",@"multipart/form-data", nil];
            [manager.requestSerializer willChangeValueForKey:@"timeoutInterval"];
            manager.requestSerializer.timeoutInterval = 15.0f;
